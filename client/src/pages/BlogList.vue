...
.clear-filter { margin-top: 0.5rem; background: none; border: 1px solid var(--color-border); border-radius: var(--radius); padding: 0.2rem 0.8rem; cursor: pointer; font-size: 0.8rem; color: var(--color-text-secondary); font-family: var(--font-sans); transition: border-color 0.4s ease, color 0.4s ease; }
.clear-filter:hover { border-color: var(--color-accent); color: var(--color-accent); }

/* 文章列表 stagger 入场 */
.post-list :deep(.post-card:nth-child(1)) { animation-delay: 0.15s; }
.post-list :deep(.post-card:nth-child(2)) { animation-delay: 0.22s; }
.post-list :deep(.post-card:nth-child(3)) { animation-delay: 0.29s; }
.post-list :deep(.post-card:nth-child(4)) { animation-delay: 0.36s; }
.post-list :deep(.post-card:nth-child(5)) { animation-delay: 0.43s; }
</style>
