...
import { renderMarkdown, setupCopyDelegate } from '../utils/markdown'

const props = defineProps<{ content: string }>()
const rendered = computed(() => renderMarkdown(props.content))
const containerRef = ref<HTMLElement>()
onMounted(() => {
  if (containerRef.value) setupCopyDelegate(containerRef.value)
})
</script>
...
.markdown-body :deep(.code-block:hover .copy-btn) {
  opacity: 1;
}
